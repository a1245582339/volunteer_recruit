...
<div v-loading.body="listLoading">
  <el-form :model="reg" ref="changeInfo" label-width="30px" status-icon :rules="checkReg">
    ...
    <el-form-item label="修改密码" :label-width="formLabelWidth" prop="chanegPass" style="width: 500px;">
    ...
    <el-form-item label="手机号" :label-width="formLabelWidth" prop="tel" style="width: 500px;">
...
export default {
  ...
  created() {
    var vm = this
    this.$emit('listenActiveIndex', 'me')
  ...
  methods: {
    ...
    checkCurrentPass() {
      ...
      checkCurrentPassApi({
        id: vm.userInfo.stu_id,
        password: vm.reg.currentPass
      })
        .then(response => {
    ...
    AvatarSuccess(res, file) {
      updateAvatar({ id: this.userInfo.stu_id, avatar: res.data.path }).then(
        response => {
